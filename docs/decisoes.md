...
Para atender à exigência de rastreabilidade (sem exclusão silenciosa via DELETE), a consolidação cria a coluna de diagnóstico status_registro, que mapeia as seguintes anomalias na ordem:REJEITADO_ESTORNO: se fare_amount < 0 ou total_amount < 0;REJEITADO_TEMPO_INVALIDO: se dropoff_datetime <= pickup_datetime;REJEITADO_DISTANCIA_ZERO: se trip_distance = 0 com fare_amount > 0;REJEITADO_PASSAGEIRO_INVALIDO: se passenger_count for nulo ou  <=0 (evitando viagens técnicas/fantasmas);Registros que passam em todas as validações recebem a marcação 'APROVADO'.
`rejected_yellow_trips` cria a tabela dedicada de quarentena via CREATE OR REPLACE TABLE, filtrando exclusivamente as linhas onde status_registro LIKE 'REJEITADO%'. A tabela armazena os atributos originais da viagem, a coluna motivo_rejeicao (status_registro) 
`approved_yellow_trips` materializa a tabela refinada contendo apenas as corridas marcadas como 'APROVADO', prontas para consumo na camada de Analytics/BI sem a necessidade de cláusulas WHERE repetitivas ou filtros de tratamento de erro. Com essa divisão, garante-se matematicamente que COUNT(approved_yellow_trips) + COUNT(rejected_yellow_trips) = COUNT(stg_yellow_trips), assegurando 100% de auditabilidade e integridade no pipeline.