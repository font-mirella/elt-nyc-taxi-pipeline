...
Validado rodando o pipeline duas vezes: `fato_corrida` produz sempre 2.964.550 linhas com os mesmos `id_corrida_sk` (idempotente, LED-35), e a reconciliação bate exata — `SUM(total_amount)` de `raw_trips`menos `fato_corrida` é 2.318,78, igual à soma  de `rejected_trips.total_amount` (LED-34). Nenhum FK da fato ficou nulo.