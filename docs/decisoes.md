...
2. **`dim_atributos_corrida` não usa chaves reservadas -1/-2 literais**, apesar de `docs/modelagem_dimensional.md` §2.4 pedir isso genericamente. O grupo Flex Fare (ratecode_id/store_and_fwd_flag NULL) ocorre nos 3 vendors com volumes bem diferentes; colapsar num único `id_atributos_sk = -1` universal perderia essa distinção sem necessidade. Como cada combinação observada (25 no total, incluindo as com atributo nulo) já vira uma linha própria com chave substituta sequencial, a fato nunca precisa de FK nula — o requisito de fundo fica satisfeito. `ratecode_id` NULL e `= 99` recebem só o rótulo descritivo ("Não se aplica"/"Desconhecido").
