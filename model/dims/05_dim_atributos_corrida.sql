...
--
-- Sobre os valores reservados (docs/modelagem_dimensional.md §2.4): o grupo Flex Fare
...
-- essa distinção sem necessidade, então cada combinação observada mantém linha própria com
-- chave sequencial e a sentinela entra na COLUNA DE ATRIBUTO: ratecode_id = -1 e
-- store_and_fwd_flag = 'N/A'. Assim nenhuma coluna da dimensão fica anulável, e um filtro
-- negativo comum deixa de descartar essas 140.114 linhas em silêncio por lógica ternária. 
-- "Desconhecido" dispensa sentinela: RatecodeID = 99 já é o código de "Null/unknown" da 
-- própria fonte.
...
SELECT
    ...
    ratecode_id,
    CASE
        WHEN ratecode_id = -1 THEN 'Não se aplica'       
    ...
    store_and_fwd_flag,
    CASE
        WHEN store_and_fwd_flag = 'N/A' THEN 'Não se aplica'   
...
FROM (
    SELECT DISTINCT
        ...
        COALESCE(store_and_fwd_flag, 'N/A') AS store_and_fwd_flag
    FROM approved_trips
) combos
